...
const PopUpForEdit = ({ onClose, blogId }) => {
    ...
    useEffect(() => {
        const fetchBlogData = async () => {
            try {
                const response = await AuthAPI.get(`${process.env.NEXT_PUBLIC_BASE_URL}/blog/${blogId}`);
                setBlogData(response.data);
                setEditedTitle(response.data.title);
                setEditedDetail(response.data.detail);
                console.log(response.data);
            } catch (error) {
                Notify('Error fetching blog data:', error);
            }
        };
